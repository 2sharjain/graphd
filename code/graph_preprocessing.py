...
def getTime(g,Nmax):
    ...
    #     a = tlp.node(i)
    #     time_steps = max(time_steps,int(labels[a].split('_')[0]))
    time_steps = 11
    for i in range(Nmax):
        temp = []
...
def AM2DM(AM,g, fill_value=0):
    ...
    if(type(fill_value) == int):
        ...
        DM[DM == tmp] = np.max(DM)
    labels = g.getStringProperty('label')
    for a in range(g.numberOfNodes()):
        for b in range(g.numberOfNodes()):
            #if a in g.nodes():
            i = tlp.node(a)
            #else:
            #    continue 
            ...
            #else:
            #    continue
            label_i = int(labels[i].split('_')[0])
            label_j = int(labels[j].split('_')[0])
            DM[a][b] -= abs(label_i-label_j)
    # print("PROGRESS")
    return DM
...
def graph2predictData(g, graph_id, N_max, max_deg, model_inputs, features_names, topology_fn, swap=True, scalers=[]):
    ...
    if("time" in model_inputs):
        times = getTime(g,N_max)
        input_dic["time"] = times
