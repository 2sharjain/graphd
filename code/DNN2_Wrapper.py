...
class DNN2_Wrapper:
    def __init__(self, exec_name, Nmax, graph_features, batch_size, topology_fn, K, model_fn, loss_name="stress", opt=Adam(learning_rate=0.001), model_weights_path=""):
        ...
        if(loss_name == "tsnet"):
            # l_kl, l_c, l_r = 1., 0.5, 0.7      # lambda for tsnet stage 1 - this is og
            # l_kl, l_c, l_r = 0., 0.5, 0.7      # lambda for tsnet stage 1 -  this is _ new
            # l_kl, l_c, l_r = 1, 0.05, 0.8
            l_kl, l_c, l_r = 10, 0, 5   # this is the god of parameters, hail this worship this. Sell your soul to these hyperparameters
            # l_kl, l_c, l_r = 0, 10, 0
            r_eps=0.05
            l_sum = l_kl + l_c + l_r
            # l_kl /= l_sum
            ...
            loss_fn = losses.tsnet(
                ...
                self.model.input[self.model_inputs.index("time")], 
                Nmax, 
    ...
    def train(self, dirs_prefix, patience=20, max_epochs=1000, monitoring="val_loss", visualCallback=True):
