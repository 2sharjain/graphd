...
def tsnet(y_pred, DM, real_nodes_mask, sigmas, times, n_nodes, l_kl=None, l_c=None, l_r=None, r_eps=0.05, epsilon=1e-16, batch_size=8):
    ...
    #X_DM = layers.Lambda(lambda x: tf.identity(x))(DM)
    local_real_nodes_mask = K.cast(tf.identity(real_nodes_mask), dtype)
    #local_real_nodes_mask = layers.Lambda(lambda x: tf.identity(x))(real_nodes_mask)
    matrix_real_nodes_mask = local_real_nodes_mask * tf.transpose(local_real_nodes_mask, perm=(0,2,1))
    #transpose = layers.Lambda(lambda x: tf.transpose(x, perm=(0, 2, 1)))
    #transposed_mask = transpose(local_real_nodes_mask)
    #matrix_real_nodes_mask = layers.Multiply()([local_real_nodes_mask, transposed_mask])


    local_time_nodes_mask = K.cast(tf.identity(times), dtype)
    #matrix_time_nodes_mask = local_time_nodes_mask * tf.transpose(local_time_nodes_mask, perm=(0,2,1))
    #matrix_time_nodes_mask = tf.matmul(local_time_nodes_mask, local_time_nodes_mask, transpose_b=True)
    matrix_time_nodes_mask = tf.matmul(local_time_nodes_mask, tf.transpose(local_time_nodes_mask, perm=(0,2,1)))
    ...
    repulsion = tf.reshape(K.cast(-(1 / (2 * real_n**2)), dtype), (batch_size,1)) * K.sum(tf.linalg.set_diag(K.log(euclidean_var(Y_pos,matrix_time_nodes_mask)+r_eps), tf.zeros((batch_size,N)))*matrix_time_nodes_mask, axis=2)
