...
def graph2trainingData(g, graph_id, doSeed, N_max, K, model_inputs, features_names, topology_fn, swap=True):
    ...
    for i in model_inputs:
        if(i == "DM"):
            DM = preprocess.AM2DM(AM,g, fill_value=-1)
            if(swap):
            ...
            if(swap):
                ...
                cur_nodes_mask = np.zeros((AM.shape[0], 1))
                cur_nodes_mask[:real_n_nodes] = 1
            # print(f"dashfdsnfdjnfdasjfdasjfldasnfasnfldsnfdkashfldshfkdas\n\n\n\n\n\n\n{cur_nodes_mask.shape}")
            inputs.append(cur_nodes_mask)
        elif(i=='time'):
            times = preprocess.getTime(g,N_max)
            inputs.append(times)
    g = None
    inputs = tuple(inputs)
